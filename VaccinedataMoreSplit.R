...
caseData %>%
    ...
    summarise(`all` = sum(`newCasesBySpecimenDate-0_59`+`newCasesBySpecimenDate-60+`),
              `0_59` = sum(`newCasesBySpecimenDate-0_59`),
              `60_69`=sum(`newCasesBySpecimenDate-60_64` +`newCasesBySpecimenDate-65_69`),                  `70_79`=sum(`newCasesBySpecimenDate-70_74` +`newCasesBySpecimenDate-75_79`),
...
p <- p +
   ...
   geom_segment(data=halfLivesLines, 
    ...
    geom_text(data=halfLivesLabels[halfLivesLabels$Age=="70_79_roll",], aes(x=date+3, y=next_Rolling_average, label=paste(round(Rolling_average.x,0), "days"), colour=Age), size=2.8, angle=45, check_overlap = T, hjust=1, show.legend = FALSE ) +
    geom_text(data=halfLivesLabels[halfLivesLabels$Age!="70_79_roll",], aes(x=date+3, y=Rolling_average.y, label=paste(round(Rolling_average.x,0), "days"), colour=Age), size=2.8, angle=45, check_overlap = T, hjust=0, show.legend = FALSE)
